//
//  EMMInstalledApplicationsViewController.m
//  EMMAppStore
//
...
@interface EMMInstalledApplicationsViewController ()<UIScrollViewDelegate, EMMOperationButtonDelegate>{
...
@implementation EMMInstalledApplicationsViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.navigationItem.title = @"已安装的应用";
    
    [self setScrollViewTitle];
    
    [self creatMyScrollView];
}

- (void)setScrollViewTitle{
    UIImageView *oftenIcon = [[UIImageView alloc] initWithFrame:CGRectMake(20, 20, 15, 15)];
    oftenIcon.image = [UIImage imageNamed:@"icon_app"];
    UILabel *oftenTitle = [[UILabel alloc] initWithFrame:CGRectMake(40, 20, ScreenW - 50, 15)];
    
    NSMutableAttributedString *AttributedStr = [[NSMutableAttributedString alloc]initWithString:@"常用应用  拖动可排序，点击\"x\"可删除常用"];
    
    [AttributedStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:13.0] range:NSMakeRange(4, AttributedStr.length-4)];
    [AttributedStr addAttribute:NSForegroundColorAttributeName value:[UIColor grayColor] range:NSMakeRange(4, AttributedStr.length-4)];
    
    oftenTitle.attributedText = AttributedStr;
    
    
    
    [self.myScrollView addSubview:oftenTitle];
    [self.myScrollView addSubview:oftenIcon];
}
...
- (void)creatMyScrollView{
    ...
    _gridListView.backgroundColor = [UIColor whiteColor];
    [_gridListView setFrame:CGRectMake(0, 64, ScreenWidth, GridHeight * PerColumGridCount)];
    
    [self.myScrollView addSubview:_gridListView];
...
-(void)getNewData
{
    ...
    self.myScrollView.contentSize = CGSizeMake(gridHeight, 0);
}
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self.showGridArray = [[NSMutableArray alloc] init];
        self.showGridImageArray = [[NSMutableArray alloc] init];
        self.showGridIDArray = [[NSMutableArray alloc] init];
    }
    return self;
}
